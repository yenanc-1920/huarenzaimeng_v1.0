...
function releaseJar() {
  ...
  if (jars.length !== 1) throw new Error(`PROD_JAR_IDENTITY_INVALID count=${jars.length}`)
  return join(target, jars[0])
}

function entries(jar) {
  const result = spawnSync('jar', ['tf', jar], { cwd: root, encoding: 'utf8', shell: false })
  if (result.error || result.status !== 0) throw new Error('PROD_JAR_ENTRY_READ_FAILED')
  return result.stdout.split(/\r?\n/)
}

try {
  run('full deterministic repository gate', node, ['tools/run-test-deployment-gate.mjs'])
  run('PROD startup contracts', mvn, [
    ...mavenBase, '-pl', 'apps/api', '-am',
    '-Dtest=ProdProfileApplicationSmokeTest,ProdFlywayBootstrapRunnerTest,ReleaseSecretBoundaryValidatorTest,ReleaseMigrationGateFilterTest,BuildSelectionContractTest',
    '-Dsurefire.failIfNoSpecifiedTests=false', 'test'
  ])
  run('PROD release package', mvn, [...mavenBase, '-pl', 'apps/api', '-am', 'package', '-DskipTests'])
  const jarEntries = entries(releaseJar())
  if (jarEntries.some(entry => entry.startsWith('BOOT-INF/classes/db/devdata/'))) {
    throw new Error('DEVDATA_PRESENT_IN_PROD_JAR')
  }
  for (const required of [
    'BOOT-INF/classes/application-prod-mysql.yml',
    'BOOT-INF/classes/db/migration/V14__add_v1_business_read_models.sql'
  ]) {
    if (!jarEntries.includes(required)) throw new Error(`PROD_JAR_ENTRY_MISSING ${required}`)
  }
  process.stdout.write('\nPROD_DEPLOYMENT_GATE_GO\n')
} catch (error) {
  process.stderr.write(`\nPROD_DEPLOYMENT_GATE_NO_GO ${error.message}\n`)
  process.exitCode = 1
}
