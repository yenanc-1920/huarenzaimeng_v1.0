...
FROM node:24-alpine AS v2-h5-build
WORKDIR /workspace/apps/miniapp
COPY apps/miniapp/package.json apps/miniapp/package-lock.json ./
RUN npm ci
COPY apps/miniapp/index.html apps/miniapp/tsconfig.json apps/miniapp/vite.config.ts ./
COPY apps/miniapp/src src
COPY apps/miniapp/scripts scripts
ENV VITE_H5_BASE=/h5/
RUN npm run build:h5:v2
...
COPY --from=v2-h5-build /workspace/apps/miniapp/dist/build/h5 apps/api/src/main/resources/static/h5
