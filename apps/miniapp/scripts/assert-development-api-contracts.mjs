...
const operatorBrand=read('src/domain/operator-brand.ts')
...
assert.match(recharge,/class="operator-logo"/)
assert.doesNotMatch(recharge,/class="operator-icon">号码/)
for(const operator of ['GRAMEENPHONE','ROBI','BANGLALINK','AIRTEL','TELETALK'])assert.match(operatorBrand,new RegExp(`${operator}:'\\/static\\/operators\\/`))
for(const asset of ['grameenphone.svg','robi.svg','banglalink.svg','airtel.svg','teletalk.svg'])assert.ok(existsSync(resolve(root,'src/static/operators',asset)),`operator logo missing: ${asset}`)
for(const selector of ['.primary, .secondary','.filters button','.product-tabs button','.tabs button','.side','.city-filter'])assert.ok(app.includes(selector),`vertical alignment invariant missing: ${selector}`)
