...
assert.match(list, /orderRef=\$\{encodeURIComponent\(orderRef\)\}/, 'list route must carry opaque orderRef')
...
assert.doesNotMatch(client, /mockDirectory|mockCatalog|mockEligibility|BuiltinSynthetic|BUILTIN_MOCK|PROJECT_MOCK_API/, 'runtime client must only consume development APIs backed by local database records')
...
assert.match(select, /余额[\s\S]*流量[\s\S]*套餐/, 'P011 must expose balance, data and bundle product tabs')
...
assert.match(quote, /商品信息已更新/, 'version drift must expose the forced-reselection path in user language')
...
assert.match(paymentTemplate, /订单已建立[\s\S]*尚未付款[\s\S]*当前开发环境不会调用真实微信支付或真实充值/, 'P013 must use ordinary user-facing not-paid copy')
...
for(const label of ['应付总价','充值号码','运营商标识','商品标识','面额标识','报价有效期'])assert.match(paymentTemplate,new RegExp(label),`P013 summary must render ${label}`)
...
async function proveRejectedPage(parameterId){
  ...
  assert.match(payment,/<AppHeader title="准备付款" left="返回" @left="navigateBack"\/>/,`${parameterId} page must expose safe leave without another write`)
...
const rawCatalog={supportedOperatorSetVersion:1,catalogVersion:2,operatorCode:'SYN-OP',operatorQualification:'SUPPORTED',items:[{operatorCode:'SYN-OP',productRef:'SYN-P',denominationRef:'SYN-D',itemKind:'PRESET_DENOMINATION',amountMinor:10000,currency:'BDT'}],evidenceSemantics:'LOCAL_DATABASE_STATE_NOT_EXTERNAL_OPERATOR_FACT'}
...
assert.match(client, /requestData\(`\/recovery-cases\/\$\{encodeURIComponent\(recoveryCaseRef\)\}`,'GET',undefined,buyerAuthBaseUrl\)/, 'UNKNOWN must use the original authenticated case read endpoint')
...
assert.match(client, /Authorization: `Bearer \$\{requireBuyerBearerToken\(\)\}`/, 'protected buyer requests must use the runtime Bearer token')
...
assert.doesNotMatch(lifeContentDetail,/图片暂时无法显示|图片未加载/,'P043 V1 must not render business image placeholder copy')
