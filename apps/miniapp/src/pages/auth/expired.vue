...
const allowedReturnTargets=new Set(['/pages/profile/index','/pages/order/list'])
...
async function signIn(){
  ...
  try{
    ...
    uni.reLaunch({url:returnTo.value})
  }catch(cause){
...
onLoad((query)=>{const target=typeof query?.returnTo==='string'?decodeURIComponent(query.returnTo):'';if(allowedReturnTargets.has(target))returnTo.value=target})
</script>
