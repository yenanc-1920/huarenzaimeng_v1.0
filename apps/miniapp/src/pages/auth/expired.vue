...
<template><view class="page"><AppHeader left="关闭" @left="uni.reLaunch({url:'/pages/index/index'})"/><view class="content center"><view class="identity">⌁</view><view class="heading" role="heading" aria-level="1">微信快捷登录</view><text class="desc">登录后可查看自己的订单并继续充值服务。登录与找回历史订单相互独立。</text><text v-if="error" class="risk">{{error}}</text></view><view class="action"><button class="primary" :disabled="loading" @click="signIn">{{loading?'登录中…':'微信快捷登录'}}</button><button class="secondary" @click="uni.navigateTo({url:'/pages/order/recovery'})">找回历史订单</button></view></view></template><style src="../../styles/shared.css"></style><style scoped>.center{text-align:center}.identity{width:120rpx;height:120rpx;border-radius:36rpx;background:#edf3fc;color:var(--brand);font-size:50rpx;display:flex;align-items:center;justify-content:center;margin:20rpx auto}.risk{display:block;color:var(--risk);margin-top:24rpx}</style>
