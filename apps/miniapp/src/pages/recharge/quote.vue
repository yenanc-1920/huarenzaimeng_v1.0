...
<template><view class="page"><AppHeader title="确认充值" left="返回" @left="navigateBack"/><view class="content"><view class="progress"><text/><text/><text class="active"/></view><text class="step">第 3 步，共 3 步</text><view class="heading" role="heading" aria-level="1">请核对充值信息</view><StatusNotice v-if="error" :tone="creationPending?'unknown':'risk'" :title="creationPending?'订单结果待确认':'暂时不能继续'">{{error}}</StatusNotice><view v-if="quote" class="summary"><view><text>充值号码</text><text>{{quote.maskedPhone}}</text></view><view><text>运营商</text><text>{{quote.operatorName}}</text></view><view><text>充值商品</text><text>{{quote.productName}}</text></view><view><text>到账内容</text><text>{{money(quote.faceValue.minor)}} {{quote.faceValue.currency}}</text></view></view><view v-if="quote" class="total-card"><text>应付金额</text><strong>{{displayMoney(quote.total)}}</strong><small>已包含全部费用</small></view><text class="notice">请确认号码、运营商和充值商品无误。商品或价格发生变化时，需要返回重新选择。</text><StatusNotice v-if="quote&&!snapshotReady" tone="risk" title="商品信息已更新">请返回重新选择后再继续。</StatusNotice></view><view class="action"><button class="primary" :disabled="!snapshotReady||loading" @click="createOrder">{{loading?'正在确认…':creationPending?'查询订单创建结果':quote?`确认并创建订单 ${displayMoney(quote.total)}`:'确认并创建订单'}}</button></view></view></template>
