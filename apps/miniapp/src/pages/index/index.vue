...
const liveDhaka=ref('--:--'),liveBeijing=ref('--:--')
...
function openWinla(){
  try{globalThis.location.assign(WINLA_URL)}
  catch(error){console.error('[WINLA_NAVIGATION_FAILED]',error);uni.showToast({title:'第三方服务暂时无法打开，请稍后再试',icon:'none'})}
}

function tickClocks(){
  if(!temporalOverview.value){liveDhaka.value='--:--';liveBeijing.value='--:--';return}
  const instant=new Date(Date.now()+serverOffsetMs)
  const format=(timeZone:string)=>new Intl.DateTimeFormat('zh-CN',{timeZone,hour:'2-digit',minute:'2-digit',hour12:false}).format(instant)
  liveDhaka.value=format('Asia/Dhaka');liveBeijing.value=format('Asia/Shanghai')
}
function startClock(){if(clockTimer)clearInterval(clockTimer);tickClocks();clockTimer=setInterval(tickClocks,60000)}
...
async function loadTemporal(trigger:TemporalReadTrigger){
  ...
  if(temporalOverview.value){serverOffsetMs=Date.parse(temporalOverview.value.referenceInstant)-Date.now();startClock()}
    ...
    <view class="content">
      ...
      <view class="temporal-card" role="region" aria-label="今日时间与节假日">
        ...
        <view class="clock-grid">
          <view class="clock-cell"><text class="clock-city">达卡</text><text class="clock-time">{{liveDhaka}}</text><text v-if="temporalState==='STALE'" class="not-current">非当前时间</text><text v-else-if="temporalOverview?.clocks.dhaka.availabilityState==='UNAVAILABLE'" class="not-current">暂不可用</text></view>
          <view class="clock-cell"><text class="clock-city">北京</text><text class="clock-time">{{liveBeijing}}</text><text v-if="temporalState==='STALE'" class="not-current">非当前时间</text><text v-else-if="temporalOverview?.clocks.beijing.availabilityState==='UNAVAILABLE'" class="not-current">暂不可用</text></view>
        </view>
...
<style scoped>
.page{height:100vh;height:100dvh;min-height:0;padding-bottom:calc(112rpx + env(safe-area-inset-bottom));overflow:hidden}.content{height:100%;overflow:hidden;display:flex;flex-direction:column;padding:clamp(10px,2dvh,20px) 40rpx clamp(6px,1.2dvh,16px)}
...
@media (max-height:900px){.welcome .heading{font-size:44rpx;min-height:0;margin-top:4rpx}.hero{margin-top:16rpx;padding:24rpx 32rpx;border-radius:30rpx}.hero-title{font-size:36rpx;margin-top:6rpx}.hero-copy{margin-top:4rpx}.hero-action{min-height:56rpx;margin-top:14rpx;padding:9rpx 28rpx}.hero-responsibility{margin-top:8rpx;line-height:1.35}.temporal-card{margin-top:14rpx;padding:18rpx 22rpx}.temporal-status{margin-top:6rpx}.clock-grid{margin-top:6rpx}.clock-cell{padding:10rpx 16rpx}.clock-time{font-size:34rpx}.temporal-updated{margin-top:6rpx}.holiday-list{margin-top:8rpx}.holiday-row{padding:8rpx 0}.holiday-meta{margin-top:2rpx;line-height:1.25}.temporal-note{margin-top:4rpx;line-height:1.3}.legal-links{margin:8rpx 0 0}.legal-links button{padding:6rpx 12rpx}}
@media (max-height:680px){.content{padding-top:8px}.welcome>text{font-size:22rpx}.welcome .heading{font-size:36rpx;line-height:1.2}.hero{margin-top:10rpx;padding:16rpx 26rpx}.eyebrow{font-size:20rpx}.hero-title{font-size:30rpx}.hero-copy{font-size:21rpx;line-height:1.3}.hero-action{min-height:48rpx;margin-top:8rpx;padding:6rpx 24rpx}.hero-responsibility{font-size:18rpx;margin-top:5rpx}.temporal-card{margin-top:8rpx;padding:12rpx 18rpx}.temporal-title{font-size:26rpx}.temporal-refresh{min-height:34px}.temporal-status{font-size:19rpx}.clock-cell{padding:6rpx 14rpx}.clock-time{font-size:30rpx}.holiday-row{padding:5rpx 0}.holiday-copy{font-size:20rpx}.holiday-meta,.temporal-updated,.temporal-note{font-size:16rpx}.legal-links{margin-top:4rpx}.legal-links button{font-size:18rpx;padding:3rpx 10rpx}}
</style>
