...
<template>
  <view class="page" data-page-root="order-detail" data-page-id="UX-P021" :data-view-state="state.viewState" :data-projection-version="projection?.projectionVersion??0" :data-detail-count="projection?1:0" :data-it-completed="itCompleted" :data-it-old-content-restored="itOldContentRestored" :data-it-delay-plan="itDelayPlan" data-write-eligibility="0">
    ...
    <view class="content">
      ...
      <view v-else-if="state.viewState==='READ_ERROR'" data-status-panel="READ_ERROR"><StatusNotice tone="risk" title="暂时无法读取" aria-label="暂时无法读取" role="alert">旧详情已收起，你可以重新读取或返回订单列表。</StatusNotice></view>
...
<view class="kv"><text>充值商品</text><text>{{projection.priceSnapshotSummary.productDisplayName}}</text></view>
        ...
        <view class="card facts-card" data-fact-summary="1">
          ...
          <text v-if="projection.confirmedItems.length" class="fact-line">已经完成：{{projection.confirmedItems.map(item=>itemCopy[item]).join('、')}}</text>
          <text v-if="projection.unknownItems.length" class="fact-line unknown">等待确认：{{projection.unknownItems.map(item=>itemCopy[item]).join('、')}}</text>
          <text class="fact-line">后续安排：{{responsibilityCopy[projection.responsibilityCode]}}</text>
        ...
        <view class="card timeline-card" data-timeline="1" role="list" aria-label="订单处理时间线">
    ...
    <view class="action" data-action-container="readonly">
      ...
      <button v-if="state.viewState==='READY'&&supportRef" data-action-code="OPEN_SUPPORT" class="secondary" role="button" tabindex="0" aria-label="打开微信客服" open-type="contact">打开微信客服</button>
