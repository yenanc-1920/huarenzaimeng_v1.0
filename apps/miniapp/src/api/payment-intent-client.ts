...
export const paymentIntentApi = {
  async create(orderRef:string,identity:{commandId:string;idempotencyKey:string},projection:OrderCreationProjection,
    ...
    if(!useProjectMockApi)return validatePaymentIntentResult(await mockCreatePaymentIntent(orderRef,command,projection),orderRef,command,frozenPriceSnapshot)
  ...
  async query(orderRef:string,identity:{commandId:string;idempotencyKey:string},sourceProjection:OrderCreationProjection,
    ...
    if(!useProjectMockApi)return validatePaymentIntentQueryResult(await mockQueryPaymentIntent(orderRef,request),orderRef,request,sourceProjection,frozenPriceSnapshot)
