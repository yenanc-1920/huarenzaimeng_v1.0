...
// Mirrors the backend CanonicalFingerprint: each UTF-8 field is prefixed by a four-byte big-endian length.
export function canonicalFingerprint(...fields:string[]):string{
  const bytes:number[]=[]
  for(const field of fields){
    const value=utf8(field),length=value.length
    bytes.push((length>>>24)&255,(length>>>16)&255,(length>>>8)&255,length&255,...value)
  }
