...
const activePage = ref<PageId>('A100')
...
const pages: Array<{ id: PageId; label: string; description: string }> = [
  { id: 'A100', label: '客服案件', description: '查看案件事实、下一复核点及只读处置状态' },
  { id: 'A110', label: '悬账与冲突', description: '按职责查看互斥的差异、账龄、责任与事实摘要' },
...
const showHeaderRefresh = computed(() => {
  ...
  if (readState.value.status === 'LOADING' || readState.value.status === 'ACCESS_DENIED') return false
...
<template>
  <main v-if="p021ItEntry" class="workspace p021-it-entry" data-it-entry="P021-ADMIN" :data-it-role="p021ItEntry.role">
