...
/** One-shot schema migration for the isolated CloudBase TEST database. */
@Component
@Profile("release-mysql & test-mysql")
@ConditionalOnProperty(name = "hz.environment.migration-enabled", havingValue = "true")
@Order(Ordered.HIGHEST_PRECEDENCE)
final class TestFlywayMigrationRunner implements ApplicationRunner {
    private final DataSource dataSource;
    private final Flyway flyway;
    private final String expectedDatabase;

    TestFlywayMigrationRunner(DataSource dataSource,
    ...
    public void run(ApplicationArguments args) throws Exception {
        if (expectedDatabase == null || expectedDatabase.isBlank()) {
            throw new IllegalStateException("TEST_DATABASE_NAME_REQUIRED");
        }
        try (Connection connection = dataSource.getConnection();
            ...
            if (!result.next() || !expectedDatabase.equals(result.getString(1)) || result.next()) {
                throw new IllegalStateException("TEST_DATABASE_IDENTITY_MISMATCH");
            }
        }
        flyway.migrate();
    }
