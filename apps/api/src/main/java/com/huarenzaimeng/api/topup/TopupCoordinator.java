...
public final class TopupCoordinator {
    ...
    public View submitForBuyer(SubmitCommand requested,String buyerSubjectRef) {
        ...
        TopupProviderPort.Command command=new TopupProviderPort.Command(trusted.merchantOrderRef(),requested.requestRef(),trusted.providerSku(),BangladeshPhoneNumber.normalize(trusted.recipient()),trusted.faceValueMinor(),trusted.targetCurrency(),requested.requestDigest());
    ...
    public View callback(TopupProviderPort.CallbackEnvelope envelope){TopupProviderPort.CallbackResult r;try{r=provider.verifyCallback(envelope);}catch(RuntimeException e){r=new TopupProviderPort.CallbackUnknown("CALLBACK_UNKNOWN");}if(r instanceof TopupProviderPort.InvalidCallback invalid)throw new Conflict(invalid.reasonCode());if(r instanceof TopupProviderPort.CallbackUnknown)throw new Conflict("TOPUP_CALLBACK_UNKNOWN");TopupProviderPort.VerifiedCallback v=(TopupProviderPort.VerifiedCallback)r;View current=store.require(v.merchantOrderRef());if(current.providerRef()!=null&&!current.providerRef().equals(v.providerRef()))throw new Conflict("TOPUP_CALLBACK_PROVIDER_CONFLICT");State target=state(v.state());boolean complete=v.amountMinor()!=null&&v.currency()!=null&&v.providerSku()!=null&&v.recipient()!=null;if(target==State.DELIVERED&&(!complete||v.amountMinor()!=current.reservedMinor()||!v.currency().equals(current.reserveCurrency())||!v.providerSku().equals(current.providerSku())||!BangladeshPhoneNumber.normalize(v.recipient()).equals(current.recipient())))target=State.UNKNOWN;store.callback(v,target);return store.require(v.merchantOrderRef());}
    ...
    private static void validate(TopupProviderPort.Command c){if(c==null||blank(c.merchantOrderRef())||blank(c.requestRef())||blank(c.providerSku())||blank(c.recipient())||c.faceValueMinor()<=0||c.targetCurrency()==null||!c.targetCurrency().matches("[A-Z]{3}")||c.requestDigest()==null||!c.requestDigest().matches("[a-f0-9]{64}"))throw new Conflict("TOPUP_COMMAND_INVALID");}
