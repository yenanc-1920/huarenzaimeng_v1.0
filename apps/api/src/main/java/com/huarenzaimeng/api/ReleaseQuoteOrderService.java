...
final class ReleaseQuoteOrderService {
    ...
    static String normalizeBangladeshPhone(String raw){String digits=raw==null?"":raw.replaceAll("[^0-9]","");if(digits.startsWith("00880"))digits=digits.substring(2);if(digits.startsWith("880"))digits=digits.substring(3);if(digits.startsWith("0"))digits=digits.substring(1);if(!digits.matches("1[3-9][0-9]{8}"))throw new FlowRejectedException("BANGLADESH_PHONE_INVALID");return "+880"+digits;}
