...
class JdbcWeChatPayStore implements WeChatPayCoordinator.Store {
    ...
    public WeChatPayCoordinator.View require(String ref){return jdbc.query("SELECT merchant_order_ref,request_digest,buyer_subject_ref,quote_ref,price_snapshot_digest,amount_minor,currency,provider_ref,state_code,evidence_ref,query_budget_remaining,query_deadline,refunded_minor,aggregate_version,updated_at,prepay_timestamp,prepay_nonce,prepay_package,prepay_sign_type,prepay_pay_sign,prepay_expires_at FROM hz_payment_coordination WHERE merchant_order_ref=?",(rs,n)->new WeChatPayCoordinator.View(rs.getString(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getLong(6),rs.getString(7),rs.getString(8),WeChatPayCoordinator.State.valueOf(rs.getString(9)),rs.getString(10),rs.getInt(11),rs.getTimestamp(12).toInstant(),rs.getLong(13),rs.getLong(14),rs.getTimestamp(15).toInstant(),rs.getString(16)==null||rs.getTimestamp(21)==null||rs.getTimestamp(21).toInstant().isBefore(Instant.now())?null:new WeChatPayPort.PrepayParameters(rs.getString(16),rs.getString(17),rs.getString(18),rs.getString(19),rs.getString(20))),ref).stream().findFirst().orElseThrow(()->new WeChatPayCoordinator.Conflict("PAYMENT_NOT_FOUND"));}
    ...
    @Override @Transactional public void providerResultWithPrepay(String ref,WeChatPayPort.Accepted accepted,WeChatPayCoordinator.State state){
        WeChatPayPort.PrepayParameters p=accepted.prepayParameters();
        if(p==null){providerResult(ref,accepted.providerRef(),state,accepted.evidenceRef());return;}
        if(state!=WeChatPayCoordinator.State.PREPAY_CREATED||!"RSA".equals(p.signType())||p.timeStamp()==null||!p.timeStamp().matches("\\d+")||p.nonceStr()==null||p.nonceStr().isBlank()||p.packageValue()==null||!p.packageValue().startsWith("prepay_id=")||p.paySign()==null||p.paySign().isBlank())throw new WeChatPayCoordinator.Conflict("PAYMENT_PREPAY_PARAMETERS_INVALID");
        int changed=jdbc.update("UPDATE hz_payment_coordination SET provider_ref=?,state_code='PREPAY_CREATED',evidence_ref=?,prepay_timestamp=?,prepay_nonce=?,prepay_package=?,prepay_sign_type='RSA',prepay_pay_sign=?,prepay_expires_at=?,aggregate_version=aggregate_version+1,updated_at=CURRENT_TIMESTAMP(3) WHERE merchant_order_ref=? AND state_code='NEW'",accepted.providerRef(),accepted.evidenceRef(),p.timeStamp(),p.nonceStr(),p.packageValue(),p.paySign(),Timestamp.from(Instant.now().plusSeconds(7200)),ref);
        if(changed!=1)throw new WeChatPayCoordinator.Conflict("PAYMENT_PREPAY_STATE_CONFLICT");
    }
