...
public final class WinlaTopupAdapter implements TopupProviderPort {
    ...
    @Override public CallbackResult verifyCallback(CallbackEnvelope envelope){
        ...
        Normalized normalized=normalized(raw.providerAmountRaw(),raw.amountUnit(),raw.providerCurrencyRaw(),original.localCurrency());String recipientDigest=raw.recipientRaw()==null?original.recipientDigest():digestRecipient(raw.recipientRaw());
        boolean bound=normalized.valid()&&normalized.minor()==original.localAmountMinor()&&(raw.providerSku()==null||original.providerSku().equals(raw.providerSku()))&&(raw.recipientRaw()==null||original.recipientDigest().equals(recipientDigest))&&original.providerRef().equals(raw.providerRef())&&original.merchantOrderRef().equals(raw.merchantOrderRef());
        ProviderExchangeStore.NormalizationStatus status=status(bound,raw.stateRaw(),normalized);
        record(raw.providerRequestRef(),raw.merchantOrderRef(),original,raw.providerAmountRaw(),raw.providerCurrencyRaw(),raw.providerSku()==null?original.providerSku():raw.providerSku(),recipientDigest,raw.rawResponseDigest(),raw.providerRef(),raw.observedAt(),normalized,status,ProviderExchangeStore.Operation.CALLBACK);
        ...
        return state==null?new CallbackUnknown("WINLA_CALLBACK_STATE_UNKNOWN"):new VerifiedCallback(raw.callbackId(),raw.callbackDigest(),raw.merchantOrderRef(),raw.providerRef(),state,normalized.minor(),normalized.currency(),original.providerSku(),raw.recipientRaw()==null?null:BangladeshPhoneNumber.normalize(raw.recipientRaw()),raw.observedAt(),raw.evidenceRef());
    }
    private Result normalize(RawResult raw,Command command,ProviderExchangeStore.Original original,ProviderExchangeStore.Operation operation){
        long localMinor=command!=null?command.providerAmountMinor():original.localAmountMinor();String localCurrency=command!=null?command.providerCurrency():original.localCurrency();String merchantOrder=command!=null?command.merchantOrderRef():original.merchantOrderRef();String sku=command!=null?command.providerSku():original.providerSku();String recipientDigest=command!=null?digestRecipient(command.recipient()):original.recipientDigest();String requestDigest=command!=null?command.requestDigest():original.canonicalRequestDigest();
        Normalized normalized=normalized(raw.providerAmountRaw(),raw.amountUnit(),raw.providerCurrencyRaw(),localCurrency);boolean bound=normalized.valid()&&normalized.minor()==localMinor&&merchantOrder.equals(raw.merchantOrderRef())&&(raw.providerSku()==null||sku.equals(raw.providerSku()))&&(raw.recipientRaw()==null||recipientDigest.equals(digestRecipient(raw.recipientRaw())))&&(operation==ProviderExchangeStore.Operation.SUBMIT||Objects.equals(original.providerRef(),raw.providerRef()));
        ...
        record(raw.providerRequestRef(),merchantOrder,base,raw.providerAmountRaw(),raw.providerCurrencyRaw(),raw.providerSku()==null?sku:raw.providerSku(),recipientDigest,raw.rawResponseDigest(),raw.providerRef(),raw.observedAt(),normalized,status,operation);
    ...
    private Normalized normalized(String amountRaw,AmountUnit unit,String currencyRaw,String expectedCurrency){String currency=currencyRaw==null?null:currencyRaw.trim().toUpperCase(Locale.ROOT);if(!expectedCurrency.equals(currency)||!contract.currency().equals(currency))return Normalized.invalid(ProviderExchangeStore.NormalizationStatus.INVALID_CURRENCY);try{if(amountRaw==null||unit==null||!amountRaw.matches(unit==AmountUnit.MINOR?"[0-9]+":"[0-9]+(?:\\.[0-9]+)?"))return Normalized.invalid(ProviderExchangeStore.NormalizationStatus.INVALID_AMOUNT);long minor=unit==AmountUnit.MINOR?new BigDecimal(amountRaw).longValueExact():new BigDecimal(amountRaw).movePointRight(contract.scale()).setScale(0,RoundingMode.UNNECESSARY).longValueExact();return minor<=0?Normalized.invalid(ProviderExchangeStore.NormalizationStatus.INVALID_AMOUNT):new Normalized(minor,currency,ProviderExchangeStore.NormalizationStatus.VALIDATED);}catch(ArithmeticException invalid){return Normalized.invalid(ProviderExchangeStore.NormalizationStatus.INVALID_AMOUNT);}}
    ...
    public enum AmountUnit{MAJOR,MINOR}
    public record RawResult(String providerRequestRef,String merchantOrderRef,String providerRef,String stateRaw,String providerAmountRaw,AmountUnit amountUnit,String providerCurrencyRaw,String providerSku,String recipientRaw,String rawResponseDigest,Instant observedAt,String evidenceRef,String reasonCode){}
