...
class JdbcWeChatPayStoreConcurrencyTest {
    @Test void competingTerminalObservationsCommitExactlyOneState() throws Exception {
        ...
        for(int i=0;i<20;i++){
            String ref="ORDER-"+i;
            jdbc.update("INSERT INTO hz_payment_coordination VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",ref,"a".repeat(64),"BUYER","QUOTE","b".repeat(64),1000,"CNY",null,"UNKNOWN",null,2,Timestamp.from(Instant.now().plusSeconds(60)),0,1,Timestamp.from(Instant.now()));
    ...
    private static void insertUnknownRefund(JdbcTemplate jdbc,String order,String refund){
        ...
        jdbc.update("INSERT INTO hz_payment_coordination VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",order,"a".repeat(64),"BUYER","QUOTE","b".repeat(64),400,"CNY","P","REFUND_PROCESSING",null,2,Timestamp.from(now.plusSeconds(60)),400,1,Timestamp.from(now));
